...
class ActiveNonsProcess(
            ...
            scope.launch {
                runCatching {
                    while (process.isAlive && isActive) {
                        client.webSocket("ws://localhost:$port/pipe") {
                            ...
                            while (!stopSemaphore.tryAcquire() && isActive) {
                                ...
                                if (msgRes.isFailure || stopSemaphore.tryAcquire() || !isActive)
                                        break
                                ...
                                val response = runCatching {
                                    receiveDeserialized<SimulationApiMessage>()
                                }

                                log.trace(
                                        "Got response of type {}",
                                        (response.getOrNull()
                                                        ?: response.exceptionOrNull()!!)::class
                                                .java
                                                .name
                                )
                                launch { msg.cont.resumeWith(response) }
                            }
                    ...
                    if (process.isAlive) {
                        ...
                        .onFailure { exception ->
                            log.error("Error in runCatching loop: {}", exception.message)
                            stop()
                        }
            }

    private val overlord =
                    ...
                    val stdout =
                            runCatching {
                                process.inputStream.bufferedReader().use { it.readText() }
                                    ...
                                    .getOrElse { it.toString() }
                    log.trace("Process terminated with exit code 0. stderr+stdout:\n$stdout")
                }
            }
    ...
    override fun hibernateIfTooOld(
            ...
            dir: Path
    ): SuspendedNonsProcess? {
        return if (lastActivity < minPermittedLastActivityTimestamp) {
            runBlocking { suspend(dir) }
        } else {
            null
        }
    }
    ...
    override fun stop() {
        runCatching { stopSemaphore.release() }.onFailure {
            return
        }
        messageQueue.close()
        process.destroy()
                        ...
                        cont.resumeWithException(
                                it
                                        ?: IllegalStateException(
                                                "Cannot send message. Is NoNs process still alive?"
                                        )
                        )
                    }
    ...
    suspend fun suspend(dir: Path): SuspendedNonsProcess {
        val hibernateFile =
                withContext(Dispatchers.IO) { Files.createTempFile(dir, "simulation", ".yml") }
    ...
    private data class Msg(
            val msg: SimulationApiMessage,
            val cont: CancellableContinuation<SimulationApiMessage>
    )
}
